    ...
    public partial class FormLewtz : Form
    {
       ...
       private void btnAddTreasure_Click(object sender, EventArgs e)
        ...
        private MagicItem GenerateItem(string rank)
        {
            ...
            string abilities = "";
            string itemType = "";
            double cost = 0; //the total cost (takes into account non-enhancement bonus abilities
            ...
            do
            {
                ...
                {
                    case "Magic Armor":
                        if (rolledItem.ToString() == "Nothing") //if this has not been rolled and it gets here, it is just a simple magic armor with no abilities
                        {
                            rolledItem = (Item)ArmorTable.rdsResult.Single();
                            cost = rolledItem.GetCost();
                        }
                        /*            *        "+1"     +       " Armor Name"    = "+1 Chain Shirt"  */
            ...
            {
                cost += Math.Pow(bonus, 2) * 200000 +30000; //bonus^2 * 2000 gold
            }
            if (magicRolledItem.GetItemType() == "Magic Armor")
            {
                cost += Math.Pow(bonus, 2) * 100000 + 15000; //bonus^2 * 1000 gold
            }
